...
LteSchedulerEnb& LteSchedulerEnb::operator=(const LteSchedulerEnb& other)
{
    ...
    // Copy schedulers
    SchedDiscipline disciplineA = mac_->getSchedDiscipline(direction_);
    SchedDiscipline disciplineB = mac_->getDynSchedDiscipline();

    const CarrierInfoMap *carriers = mac_->getCellInfo()->getCarrierInfoMap();
    LteScheduler *newSched = nullptr;
    for (auto& item : *carriers) {
        newSched = getScheduler(disciplineA, disciplineB, direction_);
    ...
    // Copy Allocator
    if (disciplineA == ALLOCATOR_BESTFIT)  
...
void LteSchedulerEnb::initialize(Direction dir, LteMacEnb *mac, Binder *binder)
{
    ...
    // Create Allocator
    if (disciplineA == ALLOCATOR_BESTFIT)                                            // NOTE: create this type of allocator for every scheduler using Frequency Reuse
        allocator_ = new LteAllocationModuleFrequencyReuse(mac_, direction_);
    else
        allocator_ = new LteAllocationModule(mac_, direction_);
...
std::map<double, LteMacScheduleList> *LteSchedulerEnb::schedule()
{
    ...
    for (auto & schedulerPtr : scheduler_) {
        ...
        SchedDiscipline disciplineA = mac_->getSchedDiscipline(direction_);
        if (disciplineA == TSN_SCHEDULER || disciplineA == MIXED){
            scheduler->updateSchedulingInfo();
            EV << "________________________start StaticScheduling _______________________________" << endl;
            scheduler->schedule(true);
            EV << "___________________________end StaticScheduling ________________________________" << endl;
            EV << "________________________start RAC+RTX _______________________________" << endl;
            if (!(scheduler->scheduleRacRequests()) && !(scheduler->scheduleRetransmissions()) && disciplineA == MIXED) {
...
unsigned int LteSchedulerEnb::scheduleStaticGrant(MacCid cid, unsigned int bytes, bool& terminate, bool& active, bool& eligible, double carrierFrequency, int minMcsIndex, int staticMcsIndex, BandLimitVector *bandLim, Remote antenna, bool limitBl)
{
	...
	// Get virtual buffer reference
	// Due to static scheduling virtual buffer not available
    /*LteMacBuffer *conn = ((dir == DL) ? vbuf_->at(cid) : bsrbuf_->at(cid));
    ...
    for ( ; cw < numCodewords; ++cw) {
        EV << "LteSchedulerEnb::grant @@@@@ CODEWORD " << cw << " @@@@@" << endl;
        toServe = bytes + MAC_HEADER + RLC_HEADER_UM + PPP_TRAILER; // serve requested bytes+headers (static allocation therefore buffer not availble)
		// consider only bytes (function param) because BSR is not avaible
		//queueLength += MAC_HEADER + RLC_HEADER_UM;  // TODO RLC may be either UM or AM
        //toServe = (queueLength <= bytes) ? queueLength : bytes; // do not serve more bytes than the maximum number of bytes requested
        ...
        for (unsigned int i = 0; i < size; ++i) { // for each band
            ...
            if (allocatedCws != 0) {
                ...
                // limit eventually allocated blocks on other codeword to limit for current cw
                bandAvailableBlocks = (limitBl ? (b1 > limit ? limit : b1) : b1);
                bandAvailableBytes = mac_->getAmc()->computeBytesOnNRbs(nodeId, b, cw, bandAvailableBlocks, dir, carrierFrequency, minMcsIndex, staticMcsIndex);
            }
            else { // if limit is expressed in blocks, limit value must be passed to availableBytes function
                bandAvailableBlocks = allocator_->availableBlocks(nodeId, antenna, b);
                bandAvailableBytes = (bandAvailableBlocks == 0) ? 0 : availableBytes(nodeId, antenna, b, cw, dir, carrierFrequency, (limitBl) ? limit : -1, minMcsIndex, staticMcsIndex); // available space (in bytes)
            }
            ...
            if (allocatedCws == 0) {
                // mark here allocation
                allocator_->addBlocks(antenna, b, nodeId, uBlocks, uBytes);
                EV << "addBlocks " << antenna << b << nodeId << uBlocks << uBytes << endl;
                // add allocated blocks for this codeword
                cwAllocatedBlocks += uBlocks;
                totalAllocatedBlocks += uBlocks;
                cwAllocatedBytes += uBytes;
            ...
            // update the counter of bytes to be served
            toServe = (uBytes > toServe) ? 0 : toServe - uBytes;
            unsigned int queueLength = (uBytes > toServe) ? 0 : queueLength - uBytes;
            if (toServe == 0) {
                // all bytes booked, go to allocation
                stop = true;
                if (queueLength == 0)                                        // the connection has also terminated the buffer
		...
		// Not required anymore --> static scheduling wo access to BSR
		// number of bytes to be consumed from the virtual buffer
        /*while (!conn->isEmpty() && consumedBytes > 0) {
...
unsigned int LteSchedulerEnb::availableBytes(const MacNodeId id,
        Remote antenna, Band b, Codeword cw, Direction dir, double carrierFrequency, int limit, int minMcsIndex, int staticMcsIndex)
{
    ...
    unsigned int bytes = mac_->getAmc()->computeBytesOnNRbs(id, b, cw, blocks, dir, carrierFrequency, minMcsIndex, staticMcsIndex);
...
LteScheduler *LteSchedulerEnb::getScheduler(SchedDiscipline disciplineA, SchedDiscipline disciplineB, Direction direction_)
{
    EV << "Creating LteScheduler " << schedDisciplineToA(disciplineA) << endl;
    EV << "Creating LteScheduler " << schedDisciplineToA(disciplineB) << endl;
    ...
    switch (disciplineA) {
        ...
        case TSN_SCHEDULER:
            return new TSNScheduler(binder_, direction_);
        case DQOS:
            return new DQos(binder_, direction_);
        case MIXED:
            return new MixedScheduler(binder_, direction_, disciplineB, mac_->par("pfAlpha").doubleValue());
