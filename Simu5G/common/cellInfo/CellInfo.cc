...
void CellInfo::registerCarrier(double carrierFrequency, unsigned int carrierNumBands, unsigned int numerologyIndex, bool useTdd, unsigned int tddNumSymbolsDl, unsigned int tddNumSymbolsUl, unsigned int tddNumSlotsDl, unsigned int tddNumSlotsUl, unsigned int tddPatternPeriodicity)
{
    ...
    else
    {
        ...
        cInfo.tddPattern = computeTddPattern(useTdd, tddNumSlotsDl, tddNumSlotsUl, tddPatternPeriodicity, cInfo.slotFormat);
        carrierMap_[carrierFrequency] = cInfo;

        // store tdd pattern used by all TDD carriers
        if (tdd){
            if (!td_cell.tdd) // first tdd carrier
                td_cell = cInfo.tddPattern;
            else if (compareTDDPatterns(cInfo.tddPattern, td_cell))
...
TddPattern CellInfo::computeTddPattern(bool useTdd, unsigned int tddNumSlotsDl, unsigned int tddNumSlotsUl, unsigned int tddPatternPeriodicity, SlotFormat sf)
{
    TddPattern tf;
    if (!useTdd)
    {
